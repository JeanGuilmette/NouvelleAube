...
choix_A=("C'est compris")
effect_A = ["Population=0"]
result_A ="Cela va coûter 2000 minerais pour l'installation d'une centrale fournissant toute l'île. Vous pourrez la construire à partir du menu Recherches."
...
choix_A=("C'est compris")
effect_A = ["Population=0"]
result_A ="Lorsque vous serez prêt à acheter le STEP3000, vous pourrez ordonner la transaction à travers le menu Recherches."

options =[]
options.append(("Option A", choix_A, effect_A, result_A))


evt_STEP3000= Events.Event("2017-11-03", "STEP3000", desc, options)
#####################################
#Le sérum Utopia
##################################
...
choix_A=("C'est compris")
effect_A = ["Population=0"]
result_A ="Le docteur est même tellement convaincu du bien que va procurer le sérum à l'humanité qu'il nous offre le sérum gratuitement. Il ne vous reste plus qu'à donner l'ordre de faire injecter le sérum à la population et ce sera fait (au prix de 350 pétrole, pour la distribution). L'ordre pourra être donné via le menu de Recherches."
...
choix_B=("Que c'est fort joyeux... mais théorique. Recrutez des volontaires qui iront vérifier cette thèse avec un autre bateau que l'ARCHE. ")
effect_B = ["Population=0"]
result_B ="Dès que vous serez prêt à donner l'ordre du départ, faites le par l'entremise du menu Recherches. Cela provoquera le départ du bateau. Cependant, le bateau aura besoin de pétrole et de nourriture pour effectuer le voyage. Ainsi, en cliquant sur l’icône, vous vous départirez de 750  unités de  pétrole, de 300 d'agriculture et de 300 de pêche. "
choix_C=("Que c'est fort joyeux... mais théorique. Recrutez des volontaires qui iront vérifier cette thèse avec l'ARCHE. ")
effect_C = ["Population=0"]
result_C ="Dès que vous serez prêt à donner l'ordre du départ, faites le par l'entremise du menu Recherches. Cela provoquera le départ du bateau. Cependant, le bateau aura besoin de pétrole et de nourriture pour effectuer le voyage. Ainsi, en cliquant sur l’icône, vous vous départirez de 3500  unités de  pétrole, de 500 d'agriculture et de 500 de pêche. "
choix_D=("Honnêtement... j'aimerais bien garder le pouvoir. Ne prévenez personne que le retour pourrait être possible! Ainsi, je pourrai rester maître de l'île, étant donné qu'une fois au Canada, alors que le programme ARCHE sera terminé, la population voudra beaucoup trop certainement organiser des élections que je ne gagnerais peut-être pas!")
effect_D = ["Population=0", "Karma=1"]
result_D ="Hum... bien... c'est votre choix."
options =[]
options.append(("Option A", choix_A, effect_A, result_A))
options.append(("Option B", choix_B, effect_B, result_B))
options.append(("Option C", choix_C, effect_C, result_C))
...
#Éruption vésuvienne
##################################
desc = "Monsieur, il faut que je bous annonce que mes détecteurs ont relevé une activité sismique formidable dans les tréfonds de notre sol. Selon moi, une terrible éruption volcanique pourrait survenir d'un instant à l'autre."
