...
resourceDef = dict( \
    ...
    Energie  = dict(unitMax = 2000, delay = -310000, ratio = 100, rate = 0), #il n<y a pas vraiment de maximun d'energie prodit. Devrait etre ailleurs, avec l'argent
...
class Resource():
    ...
    def __init__(self, name):
        ...
        self.prodScience = 0.50
        self.prodBuilding = 0.5

    def AdjustProduction(self, building, science):
        self.prodBuilding += building
        self.prodScience += science

    def HourlyAdjustment(self, zoneMod, Secteur):
        prod = ((self.prodScience + self.prodBuilding) * Secteur.TypeTerrain[self.name]) * (1 - zoneMod.panique)
        self.stock += prod
        self.current -= prod
